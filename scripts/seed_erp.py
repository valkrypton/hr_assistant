...
def seed_lookups(conn):
    ...
    for role in COMPETENCY_ROLES:
        conn.execute(text("INSERT INTO competency_role (name) VALUES (:n)"), {"n": role})

    for level_name, level_val in COMPETENCY_LEVELS:
        conn.execute(
            text("INSERT INTO competency_level (name, value) VALUES (:n, :v)"),
    ...
    for title, core in DESIGNATIONS:
        conn.execute(
            text("INSERT INTO designation (name, core_name) VALUES (:n, :c)"),
            {"n": title, "c": core},
        )
...
def seed_holidays(conn):
    ...
    for reason, start, end in PAKISTANI_HOLIDAYS:
        conn.execute(
            text('INSERT INTO holiday_record (reason, start, "end") VALUES (:r, :s, :e)'),
...
def seed_people(conn, dept_ids, et_ids, desig_ids):
    ...
    for i, (gender, full_name, _origin) in enumerate(pool[:500]):
        ...
        # Employment type
        et_type = weighted_choice([t for t, _ in ET_WEIGHTS], [w for _, w in ET_WEIGHTS])
...
def seed_person_teams(conn, persons, team_ids):
    ...
    _internal_team = team_ids[16]  # "Internal Tools" (unused — index kept for reference)
            ...
            """),
                {
                    ...
                    "end": end,
                    "status": status,
                    "half_day": duration == 1 and random.random() < 0.3,
                    "taken": taken,
                },
            )
            count += 1
        ...
        for monday in mondays:
            ...
            for j, pt_id in enumerate(assigned):
                h = (
                    remaining
                    if j == len(assigned) - 1
                    else random.randint(5, max(5, remaining - 5))
                )
...
def seed_resignations(conn, persons):
    ...
    for p in exited:
        ...
        reason = (
            random.choice(RESIGNATION_REASONS)
            if sep_type == 2
        ...
        """),
            {
                "rid": role_id,
                "st": status,
                "mn": random.randint(1, 3),
                "mx": random.randint(4, 8),
...
def seed_leave_limits(conn, et_ids, lt_ids):
    ...
    _annual_id = lt_ids["Annual"]  # unused — kept as a guard that the "Annual" leave type exists
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reset", action="store_true", help="Drop and recreate all ERP tables first"
    )
