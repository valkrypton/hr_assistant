...
CORS_ALLOW_ORIGINS are misconfigured for production. These tests build fresh
Settings instances (never mutating the global `settings` singleton) to
verify each guard fires, that dev (DEBUG=true) stays zero-config, and that
APP_DATABASE_URL no longer falls back to DATABASE_URL.
"""
